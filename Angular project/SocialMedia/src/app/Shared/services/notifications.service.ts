      ...
      if (user) { 
        let _ = this.userService.getByID(user.uid as string).subscribe(currentuser => {
          ...
          if (notification.type == 0) {
            notification.ertesites =  notification.username.concat(" kedvelte a posztodat.");
            console.log(notification.ertesites);
          } else if (notification.type == 1) {
            notification.ertesites = notification.username + " követni kezdett.";
          } else {
            notification.ertesites = notification.username + " kommentelt a posztod alá.";
          }

          if(notification.ertesitestKapoUsernameId != user.uid) {
...
//   type: 0, //0 == like, 1 == kovetes 2 == komment
